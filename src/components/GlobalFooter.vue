<template>
  <div class="com-global-footer">
    ...
    <template v-for="item in advertisements" :key="item.id">
      <div
...
const fetchFriendlink = async () => {
  const res: any = await listFriendlink({
    enable: true,
    field: ['id', 'title', 'link'],
  })
  if (res.status === 200) {
    friendlinks.value = res.data.friendlink || []
  }
}

Promise.all([fetchFriendlink(), getAdvertisements('global')])
</script>
<style lang="scss">
.com-global-footer {
  .footer-friendlink {
