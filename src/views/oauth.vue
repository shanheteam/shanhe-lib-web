...
import { resolveOauthType, SSO_LOGOUT_RETURN_KEY, SSO_LOGOUT_RETURN_PATH_KEY } from '@/utils/oauth'
...
onMounted(async () => {
  ...
  if (!code) {
    // 单点登出(SLO)后 IdP 跳回本站回调地址（无 code），静默恢复到登出前的页面
    if (sessionStorage.getItem(SSO_LOGOUT_RETURN_KEY) === '1') {
      sessionStorage.removeItem(SSO_LOGOUT_RETURN_KEY)
      const returnPath = sessionStorage.getItem(SSO_LOGOUT_RETURN_PATH_KEY)
      sessionStorage.removeItem(SSO_LOGOUT_RETURN_PATH_KEY)
      router.replace(returnPath && returnPath !== '/' ? returnPath : '/')
      return
    }
    error.value = '缺少授权码'
    loading.value = false
